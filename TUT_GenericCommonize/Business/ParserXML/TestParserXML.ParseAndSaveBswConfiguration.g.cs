...
{
    [TestMethod]
    public void ParseAndSaveBswConfigurationTest_11_1()
    {
        using (ShimsContext.Create())
        {
            ...
            XElement documentcover = new XElement("AR-PACKAGES");
            ...
            documentcover.Add(document);
            XDocument bsw = new XDocument();
            bsw.Add(documentcover);

            Renesas.Generator.MCALConfGen.Business.Parser.Fakes.ShimParserXML.AllInstances.CheckERR000025ListOfBswConfig = (ParserXML instance, List<BswConfig> bswConfig) =>
            {
                return;
            };
            Renesas.Generator.MCALConfGen.Business.Parser.Fakes.ShimParserXML.AllInstances.CheckAutosarVersionBswConfig = (ParserXML instance, BswConfig bswConfig) =>
            {
                return;
            }

            ;
            Renesas.Generator.MCALConfGen.Business.Parser.Fakes.ShimParserXML.AllInstances.CheckParamConfigBswConfigStringArrayDictionaryOfStringString = (ParserXML instance, BswConfig bswConfig, String[] requiredFields, Dictionary<String, String> tagNameToField) =>
            {
                return;
            }

            ;
            Renesas.Generator.MCALConfGen.Business.Parser.Fakes.ShimParserXML.AllInstances.needVerifyVendorApiInfixBswConfig = (ParserXML instance, BswConfig bswConfig) =>
            {
                if (bswConfig.DescriptionShortName.Equals("LinInternalBehavior"))
                {
                    return true;
                }
                else
            ...
            ;
            /* Act */
            typeof(ParserXML).GetMethod("ParseAndSaveBswConfiguration", BindingFlags.Instance | BindingFlags.NonPublic, null, new Type[]{typeof(XDocument)}, null).Invoke(_target, new object[]{bsw});
            /* Assert */
            ...
            Assert.AreEqual((int)2, bswList.Count);
        }
    }


    [TestMethod]
    public void ParseAndSaveBswConfigurationTest_11_2()
    {
        using (ShimsContext.Create())
        {
            CdfDataDictionary cdfdatabase = CdfDataDictionary.Instance;
            ...
            typeof(ParserXML).GetMethod("ParseAndSaveBswConfiguration", BindingFlags.Instance | BindingFlags.NonPublic, null, new Type[] { typeof(XDocument) }, null).Invoke(_target, new object[] { bsw });
            /* Assert */
            Object cdfdata1 = typeof(ParserXML).GetField("CdfData", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(_target);
            Object bswdata1 = typeof(CdfDataDictionary).GetField("bswConfigs", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(cdfdata1);
            Dictionary<string, List<BswConfig>> bswdict1 = (Dictionary<string, List<BswConfig>>)bswdata1;
            List<BswConfig> bswList1 = bswdict1.Values.SelectMany(x => x).ToList();
            Assert.AreEqual((int)0, bswList1.Count);
        }
    }
}